  ...
  return (
    <main className="mx-auto max-w-xl px-5 py-12 sm:py-16">
      <section className="mb-8">
        <PageTitle tone="business" title="세후 실수령액 간이 계산기" subtitle="월급에서 4대보험·세금을 대략 빼고 실수령액을 확인해보세요." />
        <p className="text-sm leading-relaxed text-gray-500 text-center">회사마다 공제 구조가 다를 수 있어 정확한 급여명세서와는 차이가 있을 수 있습니다.</p>
      </section>

      <section className="rounded-[28px] border border-gray-100 bg-white p-6 shadow-sm shadow-gray-200/30 sm:p-8 space-y-4">
      ...
      {result && (
        <section className="mt-6 rounded-[28px] border border-gray-100 bg-white p-6 shadow-sm shadow-gray-200/30">
          ...
          <div className="mt-4 rounded-2xl bg-blue-50 p-4 text-center">
            <p className="text-xs font-semibold text-blue-700">최종 실수령액</p>
            <p className="mt-1 text-3xl font-extrabold text-blue-700 tabular-nums">{result.net.toLocaleString()}원</p>
          </div>
          <BottomActions onShare={async () => { if (navigator.share) await navigator.share({ title: "세후 실수령액 간이 계산기", url: shareUrl }); else { await copyToClipboardSafe(shareUrl); toast("링크를 복사했어요!"); } }} />
        </section>
      )}

      <AccordionFAQ title="세후 실수령액 간이 계산기 자주 묻는 질문" items={[
