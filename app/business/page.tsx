...
const BUSINESS_TOOLS = [
  {
    category: "핵심 도구",
    tools: [
      { title: "사장님 계산기", description: "판매가 대비 순이익 및 마진율 확인", href: "/business/profit" },
      { title: "대출부담 계산기", description: "월 소득 대비 원리금 상환 비율(DSR) 체크", href: "/business/burden" },
    ],
  },
  {
    category: "급여·노무",
  ...
  },
  {
    category: "직장인·프리",
    tools: [
      { title: "월급 실수령액", description: "비과세, 부양가족 반영 세후 급여 계산", href: "/business/salary" },
  ...
  return (
    <main className="max-w-xl mx-auto px-5 py-16 space-y-16">
      <section className="flex flex-col items-center text-center space-y-2">
        ...
        <p className="text-sm text-gray-400 font-medium">사장님을 위한 모든 정산 도구</p>
      </section>
              ...
              {group.tools.map((tool, tIdx) => (
                <Link key={tIdx} href={tool.href} className="group block bg-white border border-gray-100 p-7 rounded-[32px] hover:border-blue-600 transition-all duration-300 shadow-sm shadow-gray-200/20">
                  <div className="flex justify-between items-center text-left px-2">
                    ...
                    <span className="text-xl text-gray-200 group-hover:text-blue-600 transition-colors transform group-hover:translate-x-1">→</span>
                  </div>
                </Link>
              ))}
            </div>
